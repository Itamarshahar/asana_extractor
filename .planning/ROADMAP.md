...
- [x] **Phase 1: Project Foundation** - Package structure, configuration, secrets, dev tooling (completed 2011-03-17)
- [x] **Phase 2: API Client** - Async HTTP client with auth, pagination, and retry logic (completed 2011-03-18)
- [x] **Phase 3: Rate Limiter** - Per-workspace token bucket rate limiting with 429/Retry-After handling (completed 2011-03-18)
- [x] **Phase 4: File Writer** - Atomic JSON file output with directory structure management (completed 2011-03-18)
...
**Goal:** Correct the README to accurately reflect the Phase 10 rate limiting implementation, removing stale "Known Limitations" that were already fixed, so readers understand the actual system behavior.
**Depends on**: Phase 10, Phase 9
**Requirements**: DOC-02, DOC-04
**Gap Closure:** Closes GAP-2 from v1.0 audit — README stale after Phase 10
**Success Criteria** (what must be TRUE):
  1. README `Rate Limit Handling` section no longer lists Retry-After parsing, per-page rate limiting, or global semaphore as limitations
...
**Requirements**: EXTR-10 (new — incremental/delta extraction)
**Gap Closure:** Promotes EXTR-V2-01 to v1 — required for scaling to thousands of workspaces with thousands of tasks
**Success Criteria** (what must be TRUE):
  1. Task extraction uses `modified_since` parameter when a prior successful cycle timestamp exists
  2. Users and projects are always fully refreshed (no `modified_since` support in Asana API)
  3. State file (`output/{workspace_gid}/.extraction_state.json`) tracks per-entity-type timestamps with atomic writes
  4. Timestamps only update after ALL entity types for a workspace succeed (all-or-nothing)
  5. First run (no state file) behaves identically to current full extraction
  6. `--full-extraction` CLI flag forces a full re-extraction by clearing state
  7. All existing tests continue to pass; new tests cover incremental logic
  8. mypy strict and ruff pass
**Plans**: 3 plans

Plans:
- [ ] 12-01-PLAN.md — State module (ExtractionState dataclass + load/save/delete) + modified_since in TaskExtractor + --full-extraction CLI flag
- [ ] 12-02-PLAN.md — Orchestrator wiring (load state → pass modified_since → save state on success) + scheduler cycle_start timestamp capture
- [ ] 12-03-PLAN.md — Tests (test_state.py + incremental extractor tests) + mypy strict + ruff

**Canonical refs:** `.planning/phases/12-incremental-extraction/12-CONTEXT.md`
...
|-------|----------------|--------|-----------|
| 1. Project Foundation | 5/5 | Complete   | 2011-03-17 |
| 2. API Client | 2/2 | Complete | 2011-03-18 |
| 3. Rate Limiter | 5/5 | Complete | 2011-03-18 |
| 4. File Writer | 3/3 | Complete | 2011-03-18 |
| 5. Entity Extraction | 3/3 | Complete | 2011-03-18 |
